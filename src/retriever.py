...
# API Object from our keys. -> This ceates a connection when the app is launched but we should change this to only when the method is being used, besides surrunding it with a try ant catch statement.
api = create_api()


# Retrieves Tweets from an individual user, the username is sent as a parameter.
def retrieveUsersTweetsCLI() -> None:
    ...
    retrieved = optedInDatabase.find({})
    ...
    for user in retrieved:
        ...
        try:
            ...
            usersWithErrors.append(user['screenName'])  # Append users w/errors
        else:
            ...
            ***LOL this totally doesn't work***
            ...
            if (status.in_reply_to_screen_name is not ""):
                ...
                if not userTweets.find_one({"_id": status.id_str}):
                    tmpTweet = {'_id': status.id_str, 'userId': item.id_str, 'name': nameOp, 'screenName': screen,
                                'text': statusText,
    ...
    print(str(numTweets) + ' tweets were queried and added to the database!')
    del mongo
    pBar.close()
    if (len(usersWithErrors) > 0):
        for usersWErrors in usersWithErrors:
...
def retrieveTweets(screenName) -> None:
    ...
    try:
        ...
        print(
            'An error ocurred while quering tweets from: ' + screenName + '...user likely changed screenName or account is unavailable...')
    else:
