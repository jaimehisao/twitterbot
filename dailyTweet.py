...
def generateTweet() -> None:
    ...
    for tweet in userTweets.find({}):
        #if(tweet['screenName'] == 'anaortegon3'): #TRY AND CATCH IF ARR IS EMPTY
        if True:
    ...
    textgen = textgenrnn(name= "allTwitter_twitter")

    textgen.train_new_model(texts, context_labels = contextLabels, gen_epochs = 10, batch_size = 128, train_size= 1.28, rnn_layers = 2, rnn_size = 128, rnn_bidirectional = False, max_length = 40, dim_embeddings = 100, word_level = True)
    return textgen
